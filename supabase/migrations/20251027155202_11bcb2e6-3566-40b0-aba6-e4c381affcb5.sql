...
-- Step 2: Update user_roles table to use perfil_usuario enum instead of app_role
ALTER TABLE public.user_roles 
...
-- Step 3: Add unique constraint on user_id and role
ALTER TABLE public.user_roles
ADD CONSTRAINT user_roles_user_id_role_key UNIQUE (user_id, role);

-- Step 4: Migrate existing roles from usuarios.perfil to user_roles
INSERT INTO public.user_roles (user_id, role)
SELECT id, perfil
FROM public.usuarios
ON CONFLICT (user_id, role) DO NOTHING;

-- Step 5: Update the has_role() function to work with perfil_usuario enum
...
-- Step 6: Drop the old RLS policies on usuarios that cause infinite recursion
...
-- Step 7: Create new RLS policies using the has_role() security definer function
...
BEGIN
  ...
  -- CRITICAL: Also create entry in user_roles table
  INSERT INTO public.user_roles (user_id, role)
...
-- Step 11: Create trigger to log role changes
